...
const MAX_RESEARCH_AMOUNT=1_000_000;
routeBtn.addEventListener('click',()=>{const token=tokenInput.value.trim(),amount=Number(amountInput.value);if(!/^0x[a-fA-F0-9]{40}$/.test(token)){formStatus.textContent='Enter a valid 42-character token address';showToast('Invalid Robinhood Chain token address');return}if(!Number.isFinite(amount)||amount<=0||amount>MAX_RESEARCH_AMOUNT){formStatus.textContent='Enter an ETH amount from 0.01 to 1,000,000';showToast('Amount outside the supported research range');return}formStatus.textContent='Target accepted · swarm research can begin';showToast('Research target staged — no transaction submitted')});
